...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, player, enemy, ai_action, ai_reward, new_pos_player, new_pos_enemy, is_done):
        a = self._alpha
        g = self._gamma
        if is_done:
...
while True:
    ...
    action = P1.get_action(int(pos_player), int(pos_enemy))
    new_pos = P1.ai_step(action)
    if new_pos < 0:
        new_pos = 0
    reward = 0
    if object_player.colliderect(object_enemy):
        reward = -1
    else:
        reward = 1
    P1.update(pos_player, pos_enemy, action, reward, new_pos, pos_enemy, False)
